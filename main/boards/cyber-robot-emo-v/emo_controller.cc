...
private:
    ...
    static void ActionTask(void* arg) {
        EmoController* controller = static_cast<EmoController*>(arg);
        EmoActionParams params;
        controller->emo_.AttachServos();
            ...
            if (xQueueReceive(controller->action_queue_, &params, pdMS_TO_TICKS(1000)) == pdTRUE) {
                ...
                if (params.action_type == ACTION_SERVO_SEQUENCE) {
                            ...
                            for (int i = 0; i < array_size; i++) {
                                ...
                                if (cJSON_IsObject(action_item)) {
                                    ...
                                    if (cJSON_IsObject(osc_item)) {
                                        ...
                                        controller->emo_.Execute2(amplitude, center_angle, period, phase_diff, steps);
                                        ...
                                        controller->emo_.MoveServos(speed, servo_target);
                    ...
                    switch (params.action_type) {
                        case ACTION_WALK:
                            controller->emo_.Walk(params.steps, params.speed, params.direction,
                                                   params.amount);
                            break;
                        case ACTION_TURN:
                            controller->emo_.Turn(params.steps, params.speed, params.direction,
                                                   params.amount);
                            break;
                        case ACTION_JUMP:
                            controller->emo_.Jump(params.steps, params.speed);
                            break;
                        case ACTION_SWING:
                            controller->emo_.Swing(params.steps, params.speed, params.amount);
                            break;
                        case ACTION_MOONWALK:
                            controller->emo_.Moonwalker(params.steps, params.speed, params.amount,
                                                         params.direction);
                            break;
                        case ACTION_BEND:
                            controller->emo_.Bend(params.steps, params.speed, params.direction);
                            break;
                        case ACTION_SHAKE_LEG:
                            controller->emo_.ShakeLeg(params.steps, params.speed, params.direction);
                            break;
                        case ACTION_SIT:
#if 0
                            controller->emo_.Sit();
                        ...
                        case ACTION_SHOWCASE:
                            controller->emo_.Showcase();
                            break;
                        case ACTION_UPDOWN:
                            controller->emo_.UpDown(params.steps, params.speed, params.amount);
                            break;
                        case ACTION_TIPTOE_SWING:
                            controller->emo_.TiptoeSwing(params.steps, params.speed, params.amount);
                            break;
                        case ACTION_JITTER:
                            controller->emo_.Jitter(params.steps, params.speed, params.amount);
                            break;
                        case ACTION_ASCENDING_TURN:
                            controller->emo_.AscendingTurn(params.steps, params.speed, params.amount);
                            break;
                        case ACTION_CRUSAITO:
                            controller->emo_.Crusaito(params.steps, params.speed, params.amount,
                                                       params.direction);
                            break;
                        case ACTION_FLAPPING:
                            controller->emo_.Flapping(params.steps, params.speed, params.amount,
                                                      params.direction);
                            break;
                        case ACTION_WHIRLWIND_LEG:
                            controller->emo_.WhirlwindLeg(params.steps, params.speed, params.amount);
                            break;
                        case ACTION_HOME:
                            controller->emo_.Home();
                            break;
                    }
    ...
    void StartActionTaskIfNeeded() {
        if (action_task_handle_ == nullptr) {
            xTaskCreate(ActionTask, "emo_action", 1024 * 3, this, configMAX_PRIORITIES - 1,
                        &action_task_handle_);
        }
    }
    ...
    void QueueAction(int action_type, int steps, int speed, int direction, int amount) {
        ...
        EmoActionParams params = {action_type, steps, speed, direction, amount, ""};
    ...
    void QueueServoSequence(const char* servo_sequence_json) {
        ...
        EmoActionParams params = {ACTION_SERVO_SEQUENCE, 0, 0, 0, 0, ""};
        // 复制JSON字符串到结构体中（限制长度）
    ...
    void LoadTrimsFromNVS() {
        Settings settings("emo_trims", false);
        ...
        ESP_LOGI(TAG, "从NVS加载微调设置: 左腿=%d, 右腿=%d, 左脚=%d, 右脚=%d",
                 left_leg, right_leg, left_foot, right_foot);

        emo_.SetTrims(left_leg, right_leg, left_foot, right_foot);
    }

public:
    EmoController() {
        emo_.Init(LEFT_LEG_PIN, RIGHT_LEG_PIN, LEFT_FOOT_PIN, RIGHT_FOOT_PIN);

        LoadTrimsFromNVS();

        action_queue_ = xQueueCreate(10, sizeof(EmoActionParams));
    ...
    void RegisterMcpTools() {
        ...
        // 统一动作工具（除了舵机序列外的所有动作）
        mcp_server.AddTool("self.emo.action",
                           ...
                           "固定动作：showcase(展示动作)、home(复位)",
                           PropertyList({
                               Property("action", kPropertyTypeString, "home"),
                               Property("steps", kPropertyTypeInteger, 6, 1, 100),
                           ...
                           [this](const PropertyList& properties) -> ReturnValue {
                            ...
                            //        return true;
                                   ...
                                   return "错误：无效的动作名称。可用动作：walk, turn, jump, swing, moonwalk, bend, shake_leg, updown, whirlwind_leg, showcase, home";
                               }
                           });
        ...
        mcp_server.AddTool(
            ...
            "每个动作对象包含："
            "普通模式：'s'舵机位置对象(键名：ll/rl/lf/rf，值：0-180度)，'v'移动速度100-3000毫秒(默认1000)，'d'动作后延迟毫秒数(默认0)；"
            ...
            "重要说明：左右腿脚震荡的时候，有一只脚必须在90度，否则会损坏机器人，如果发送多个序列（序列数>1），完成所有序列后需要复位时，AI应该最后单独调用self.emo.home工具进行复位，不要在序列中设置复位参数。"
            ...
            "最后调用self.emo.home工具进行复位。"
            "振荡器模式示例："
            "示例1-双腿交替振荡（波浪效果）：{\"sequence\":\"{\\\"a\\\":[{\\\"osc\\\":{\\\"a\\\":{\\\"ll\\\":15,\\\"rl\\\":15},\\\"o\\\":{\\\"ll\\\":90,\\\"rl\\\":90},\\\"ph\\\":{\\\"rl\\\":180},\\\"p\\\":600,\\\"c\\\":3.0}}],\\\"d\\\":0}\"}；"
            "示例2-单腿振荡配合固定脚（安全）：{\"sequence\":\"{\\\"a\\\":[{\\\"osc\\\":{\\\"a\\\":{\\\"ll\\\":60},\\\"o\\\":{\\\"ll\\\":90,\\\"lf\\\":90},\\\"p\\\":400,\\\"c\\\":4.0}}],\\\"d\\\":0}\"}；"
            "示例3-快速摇摆：{\"sequence\":\"{\\\"a\\\":[{\\\"osc\\\":{\\\"a\\\":{\\\"ll\\\":30,\\\"rl\\\":30},\\\"o\\\":{\\\"ll\\\":90,\\\"rl\\\":90},\\\"ph\\\":{\\\"rl\\\":180},\\\"p\\\":300,\\\"c\\\":10.0}}],\\\"d\\\":0}\"}。",
        ...
        mcp_server.AddTool("self.emo.stop", "立即停止所有动作并复位", PropertyList(),
                           [this](const PropertyList& properties) -> ReturnValue {
                               if (action_task_handle_ != nullptr) {
        ...
        mcp_server.AddTool(
            ...
            "servo_type: 舵机类型(left_leg/right_leg/left_foot/right_foot); "
            ...
            [this](const PropertyList& properties) -> ReturnValue {
                ...
                // 获取当前所有微调值
                Settings settings("emo_trims", true);
                int left_leg = settings.GetInt("left_leg", 0);
                int right_leg = settings.GetInt("right_leg", 0);
                int left_foot = settings.GetInt("left_foot", 0);
                int right_foot = settings.GetInt("right_foot", 0);
        ...
        mcp_server.AddTool("self.emo.get_trims", "获取当前的舵机微调设置", PropertyList(),
                           [this](const PropertyList& properties) -> ReturnValue {
                               Settings settings("emo_trims", false);
        ...
        mcp_server.AddTool("self.emo.get_status", "获取机器人状态，返回 moving 或 idle",
        ...
        mcp_server.AddTool("self.emo.get_ip", "获取机器人WiFi IP地址", PropertyList(),
                           [](const PropertyList& properties) -> ReturnValue {
...
static EmoController* g_emo_controller = nullptr;

void InitializeEmoController() {
    if (g_emo_controller == nullptr) {
        g_emo_controller = new EmoController();
        ESP_LOGI(TAG, "Emo控制器已初始化并注册MCP工具");
    }
}
