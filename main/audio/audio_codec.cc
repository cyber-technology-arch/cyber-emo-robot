...
void AudioCodec::SetOutputChannels(int ch) {
    output_channels_ = ch;
    ESP_LOGI(TAG, "Set output_channels_ to %d", ch);
}

void AudioCodec::SetInputEnable(bool enable) {
     if (enable == input_enabled_) {
        return;
    }
    input_enabled_ = enable;
...
void AudioCodec::SetOutputEnable(bool enable) {
    if (enable == output_enabled_) {
        return;
    }
    output_enabled_ = enable;
