...
void CustomWakeWord::ParseWakenetModelConfig() {
    ...
    commands_.push_back({"xiao ming", "xiao ming", "wake_up"});
    commands_.push_back({"guan deng", "guan deng", "close"});
    ESP_LOGI(TAG, "自定义命名词");
    // Read custom_commands.json for additional camera commands
    if (!assets.GetAssetData("custom_commands.json", ptr, size)) {
        ESP_LOGW(TAG, "Failed to read custom_commands.json, will proceed without additional commands");
        return;
    }
    cJSON* custom_root = cJSON_ParseWithLength(static_cast<char*>(ptr), size);
    if (custom_root == nullptr) {
        ESP_LOGE(TAG, "Failed to parse custom_commands.json");
        return;
    }
    cJSON* custom_commands = cJSON_GetObjectItem(custom_root, "commands");
    if (cJSON_IsArray(custom_commands)) {
        for (int i = 0; i < cJSON_GetArraySize(custom_commands); i++) {
            cJSON* command = cJSON_GetArrayItem(custom_commands, i);
            if (cJSON_IsObject(command)) {
                cJSON* command_name = cJSON_GetObjectItem(command, "command");
                cJSON* text = cJSON_GetObjectItem(command, "text");
                cJSON* action = cJSON_GetObjectItem(command, "action");
                if (cJSON_IsString(command_name) && cJSON_IsString(text) && cJSON_IsString(action)) {
                    commands_.push_back({command_name->valuestring, text->valuestring, action->valuestring});
                    ESP_LOGI(TAG, "Custom Command: %s, Text: %s, Action: %s", command_name->valuestring, text->valuestring, action->valuestring);
                }
            }
        }
    }
...
void CustomWakeWord::Feed(const std::vector<int16_t>& data) {
    ...
    if (mn_state == ESP_MN_STATE_DETECTING) {
        ...
        for (int i = 0; i < mn_result->num && running_; i++) {
            ...
            }else if (command.action == "wake_up") {
                last_detected_wake_word_ = command.text;
                running_ = false;
