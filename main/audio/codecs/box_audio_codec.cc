...
void BoxAudioCodec::EnableOutput(bool enable) {
    ...
    if (enable) {
        ...
        ESP_LOGI(TAG, "Opening output device with sample rate: %d Hz", output_sample_rate_);
...
bool BoxAudioCodec::SetOutputSampleRate(int sample_rate) {
    // 如果采样率没变，直接返回
    if (sample_rate != -1 && sample_rate == output_sample_rate_) {
        return true;
    }
    ...
    // 调用基类方法修改 I2S 时钟
    bool ret = AudioCodec::SetOutputSampleRate(sample_rate);

    if (was_enabled) {
        // 重新开启输出，会使用更新后的 output_sample_rate_ 重新打开 esp_codec_dev
        EnableOutput(true);
    }
    return ret;
}
