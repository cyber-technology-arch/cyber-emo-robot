...
int NoAudioCodec::Write(const int16_t* data, int samples) {
    ...
    if (input_reference_) {
        ...
        if (reference_buffer_.size() > 16000) {
            reference_buffer_.clear();
            resample_index_ = 0.0f;
        }
        
        if (output_sample_rate_ > 0 && input_sample_rate_ > 0 && output_sample_rate_ != input_sample_rate_) {
            float ratio = (float)output_sample_rate_ / (float)input_sample_rate_;
            while (resample_index_ < samples) {
                int idx = (int)resample_index_;
                float frac = resample_index_ - idx;
                int16_t s1 = (int16_t)(buffer[idx] >> 16);
                int16_t s2 = (idx + 1 < samples) ? (int16_t)(buffer[idx + 1] >> 16) : s1;
                int16_t val = (int16_t)(s1 + frac * (s2 - s1));
                reference_buffer_.push_back(val);
                resample_index_ += ratio;
            }
            resample_index_ -= samples;
        } else {
            for (int i = 0; i < samples; ++i) {
                reference_buffer_.push_back((int16_t)(buffer[i] >> 16));
            }
        }
    }
...
int NoAudioCodec::Read(int16_t* dest, int samples) {
    if (input_reference_) {
        ...
        for (int i = 0; i < frames; i++) {
            ...
            int16_t mic = (value > INT16_MAX) ? INT16_MAX : (value < -INT16_MAX) ? -INT16_MAX : (int16_t)value;
            int16_t ref = 0;
            if (!reference_buffer_.empty()) {
                ref = reference_buffer_.front();
