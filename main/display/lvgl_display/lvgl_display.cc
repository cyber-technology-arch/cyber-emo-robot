...
void LvglDisplay::UpdateStatusBar(bool update_all) {
    ...
    if (board.GetBatteryLevel(battery_level, charging, discharging)) {
        // 检查电池是否充满
        bool is_full = false;
        auto power_manager = static_cast<PowerManager*>(board.GetPowerManager());
        if (power_manager) {
            is_full = power_manager->IsBatteryFull();
        }
        ...
        if (low_battery_popup_ != nullptr) {
            // if ((strcmp(icon, FONT_AWESOME_BATTERY_EMPTY) == 0 || strcmp(icon, FONT_AWESOME_BATTERY_QUARTER) == 0) && discharging) {
            if (strcmp(icon, FONT_AWESOME_BATTERY_EMPTY) == 0 && discharging) {
                ...
                // Hide the low battery popup when the battery is not low
