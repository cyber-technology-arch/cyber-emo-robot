...
void LcdDisplay::SetEmotion(const char* emotion) {
    // Normalize invalid/empty emotion early to avoid leaving a dangling image src.
    if (emotion == nullptr || emotion[0] == '\0') {
        emotion = "staticstate";
    }

    // Stop any running GIF animation and make LVGL stop referencing controller-owned buffers
    if (gif_controller_) {
        DisplayLockGuard lock(this);
        gif_controller_->Stop();
        if (emoji_image_ != nullptr) {
    ...
    if (image == nullptr) {
        ...
        // If we don't have a font-awesome fallback either, show a safe default (if available)
        // or hide both to avoid drawing a stale/dangling image source.
        if (utf8 == nullptr) {
            auto fallback = emoji_collection != nullptr ? emoji_collection->GetEmojiImage("staticstate") : nullptr;
            DisplayLockGuard lock(this);
            if (fallback != nullptr && emoji_image_ != nullptr) {
...
void LcdDisplay::SetMusicInfo(const char* song_name) {
    ...
    if (song_name != nullptr && strlen(song_name) > 0) {
        ...
        if (emoji_label_ != nullptr && lv_obj_is_valid(emoji_label_)) {
            lv_obj_clear_flag(emoji_label_, LV_OBJ_FLAG_HIDDEN);
        }
        if (preview_image_ != nullptr) {
            if (!lv_obj_is_valid(preview_image_)) {
...
void LcdDisplay::ShowSuccessScreen(const char* message) {
    DisplayLockGuard lock(this);
    
    lv_obj_t* parent = content_ ? content_ : lv_screen_active();
    if (parent == nullptr) {
        return;
    }

    // Create a container to cover the screen and show success message
    lv_obj_t* success_container = lv_obj_create(parent);
    if (content_ != nullptr) {
        lv_obj_set_size(success_container, LV_HOR_RES, LV_SIZE_CONTENT);
    } else {
        lv_obj_set_size(success_container, LV_HOR_RES, LV_VER_RES);
        lv_obj_center(success_container);
        lv_obj_move_foreground(success_container);
    }
    lv_obj_set_style_bg_color(success_container, lv_color_black(), 0);
    lv_obj_set_style_border_width(success_container, 0, 0);
    lv_obj_set_style_pad_all(success_container, 20, 0);
    ...
    // Create a checkmark icon or success label
    lv_obj_t* icon_label = lv_label_create(success_container);
    lv_label_set_text(icon_label, LV_SYMBOL_OK);
    lv_obj_set_style_text_color(icon_label, lv_color_hex(0x00FF00), 0); // Green color
    lv_obj_set_style_text_font(icon_label, &lv_font_montserrat_48, 0); // Large font for icon
    lv_obj_set_style_margin_bottom(icon_label, 20, 0);

    // Create the message label
    lv_obj_t* msg_label = lv_label_create(success_container);
    lv_label_set_text(msg_label, message);
    lv_obj_set_style_text_color(msg_label, lv_color_white(), 0);
    // Use default font if custom font is not available in base class
    lv_label_set_long_mode(msg_label, LV_LABEL_LONG_WRAP);
    lv_obj_set_style_text_align(msg_label, LV_TEXT_ALIGN_CENTER, 0);
    lv_obj_set_width(msg_label, LV_PCT(100));

    // Set custom attribute to mark bubble type
    lv_obj_set_user_data(success_container, (void*)"system");

    // Scroll to the success container if in a scrollable view
    lv_obj_scroll_to_view_recursive(success_container, LV_ANIM_ON);
}

void LcdDisplay::ShowQRCode(const char* url, const char* device_name) {
    DisplayLockGuard lock(this);

    // 直接使用当前活跃屏幕作为父容器，彻底独立全屏
    ...
    if (qr_container_ != nullptr) {
        lv_obj_del(qr_container_);
        qr_container_ = nullptr;
    }


    // ====================== 全屏黑色容器 ======================
    qr_container_ = lv_obj_create(parent);
    lv_obj_set_size(qr_container_, LV_HOR_RES, LV_VER_RES);  // 铺满整个屏幕
    lv_obj_center(qr_container_);
    lv_obj_move_foreground(qr_container_);                  // 置于最顶层，无其他界面干扰

    lv_obj_set_style_bg_color(qr_container_, lv_color_black(), 0);
    lv_obj_set_style_bg_opa(qr_container_, LV_OPA_COVER, 0);
    lv_obj_set_style_border_width(qr_container_, 0, 0);
    lv_obj_set_style_pad_all(qr_container_, 0, 0);         // 不留内边距
    lv_obj_set_style_pad_row(qr_container_, 0, 0);        // 行间距 = 0（紧贴）
    ...
    // ====================== 顶部文字：请扫描二维码进行配网 ======================
    lv_obj_t* qr_label = lv_label_create(qr_container_);
    ...
    lv_obj_set_style_margin_all(qr_label, 0, 0);
    lv_obj_set_style_pad_all(qr_label, 0, 0);
    lv_obj_set_style_margin_bottom(qr_label, 12, 0);

    // ====================== 二维码 ======================
    lv_coord_t max_usable_size = (width_ < height_ ? width_ : height_) - 80;
    lv_coord_t size = max_usable_size > 200 ? 200 : max_usable_size;
    if (size < 120) size = 120;

#if LV_USE_QRCODE
    lv_obj_t* qrcode = lv_qrcode_create(qr_container_);
    ...
    // 白色边框 + 无内边距（紧贴文字）
    lv_obj_set_style_border_width(qrcode, 2, 0);
    lv_obj_set_style_border_color(qrcode, lv_color_white(), 0);
    lv_obj_set_style_pad_all(qrcode, 0, 0);
    lv_obj_set_style_bg_color(qrcode, lv_color_white(), 0);
    lv_obj_set_style_bg_opa(qrcode, LV_OPA_COVER, 0);
    lv_obj_set_style_margin_all(qrcode, 0, 0);
#else
    lv_obj_t* qrcode = lv_label_create(qr_container_);
    lv_label_set_text(qrcode, "QR Code not supported");
#endif

    // ====================== 底部设备名 ======================
    if (device_name != nullptr) {
        lv_obj_t* device_label = lv_label_create(qr_container_);
        lv_label_set_text_fmt(device_label, "%s", device_name);
        lv_obj_set_style_text_color(device_label, lv_color_hex(0xAAAAAA), 0);
        lv_obj_set_style_margin_all(device_label, 0, 0);
        lv_obj_set_style_pad_all(device_label, 0, 0);
    }

    // 确保显示在最前面
    lv_obj_scroll_to_view_recursive(qr_container_, LV_ANIM_OFF);
}

void LcdDisplay::HideQRCode() {
    DisplayLockGuard lock(this);
