...
void CircularStrip::SetRangeColor(int start, int count, StripColor color) {
    std::lock_guard<std::mutex> lock(mutex_);
    esp_timer_stop(strip_timer_);
    for (int i = start; i < start + count && i < max_leds_; i++) {
        colors_[i] = color;
        led_strip_set_pixel(led_strip_, i, color.red, color.green, color.blue);
    }
    led_strip_refresh(led_strip_);
...
void CircularStrip::RainbowCycle(int length, int interval_ms) {
    StartStripTask(interval_ms, [this, length]() {
        static int hue = 0;
        for (int i = 0; i < max_leds_; i++) {
            int pixel_hue = (hue + i * 180 / max_leds_) % 360;
            uint8_t r, g, b;
            
            // Convert HSV to RGB
            if (pixel_hue < 120) {
                r = 255 - pixel_hue * 255 / 120;
                g = pixel_hue * 255 / 120;
                b = 0;
            } else if (pixel_hue < 240) {
                r = 0;
                g = 255 - (pixel_hue - 120) * 255 / 120;
                b = (pixel_hue - 120) * 255 / 120;
            } else {
                r = (pixel_hue - 240) * 255 / 120;
                g = 0;
                b = 255 - (pixel_hue - 240) * 255 / 120;
            }
            
            // Scale to brightness
            r = (r * default_brightness_) / 255;
            g = (g * default_brightness_) / 255;
            b = (b * default_brightness_) / 255;
