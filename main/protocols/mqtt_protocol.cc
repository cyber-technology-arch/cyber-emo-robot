...
bool MqttProtocol::StartMqttClient(bool report_error) {
    ...
    mqtt_->OnMessage([this](const std::string& topic, const std::string& payload) {
        ...
        if (strcmp(type->valuestring, "hello") == 0) {
            ...
            const char* sid_str = cJSON_IsString(session_id) ? session_id->valuestring : nullptr;
            ESP_LOGI(TAG, "Received goodbye message, session_id: %s, raw: %s", sid_str ? sid_str : "null",
                     payload.c_str());
            if (sid_str == nullptr || session_id_ == sid_str) {
                Application::GetInstance().Schedule([this]() {
                    CloseAudioChannel();
                });
            }
            // Forward to Application so goodbye + reason (e.g. unbind) can trigger EnterRebindFlow.
